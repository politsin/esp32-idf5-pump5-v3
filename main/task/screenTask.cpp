...
void screenTask(void *pvParam) {
  ...
  // Средний индикатор (FLUSH) делаем СИНИМ на экране (с учётом текущего цветопорядка)
  lv_obj_t *sqYellow = createFig(lv_color_hex(0xFF0000), btnX_start + btnSpacing);
...
static void app_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area,
  ...
  // Вернули исходные границы с +1 (ширина/высота эксклюзивные для драйвера)
